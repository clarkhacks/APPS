...
var cl=localStorage.getItem("viewmode");
document.body.classList.add(cl==null?"grid":cl);
...
var programData=[
  ...
  {name: "about.txt", url: "https://picturelements.github.io/PeNote2?url=https://picturelements.github.io/textfiles/about.txt", icon: "doc-viewer", keywords: "about,meta,info,text,document,txt,PeOS"},
  ...
  {name: "Settings", url: "", icon: {url:"cogbig"}, keywords: ""},
...
function setup(){
  ...
  setCols();
  setTimeout(function(){
  var elem=document.getElementById("loadscreen"); elem.innerHTML="<p id=\"loadmsg\">Starting up...</p><div id=\"loadercontainer\"><div class=\"loader\"></div><div class=\"loader2\"></div><div class=\"loader3\"></div>"; elem.style.backgroundColor="#268eee";},500);
  loadRepos();
  
  //create cog
  var r=19,dimple=0.8,pins=8,part=0.8,step=1/pins;
  ...
  document.getElementById("cog").setAttribute("d",out+"Z");
...
function addWindow(id,title,contStr,w,h,type){
  ...
  var inner="<div class=\"infobar infoCol\" id=\""+winCount+"\"><div class=\"close\" title=\"Close\" onclick=closeWin("+winCount+")>✕</div><div class=\"max\" title=\"Toggle\" onclick=toggle("+winCount+")>◻</div><div class=\"min\" title=\"Minimize\" onclick=minWin("+winCount+")>_</div><div class=\"reload\" title=\"Reload\" onclick=reloadWin("+winCount+")>↻</div><div class=\"winicon\"></div><div class=\"wintitle\">"+data.name+"</div></div><div class=\"content\"><iframe class=\"frame\" src=\""+(id<errID?tmpUrl:"")+"\"></iframe><div class=\"loadingoverlay\"><div class=\"loader\"></div><div class=\"loader2\"></div><div class=\"loader3\"></div></div></div><div class=\"resize\" scale=\"lw\"></div><div class=\"resize\" scale=\"lwh\"></div><div class=\"resize\" scale=\"h\"></div><div class=\"resize\" scale=\"wh\"></div><div class=\"resize\" scale=\"w\"></div>";
  elem.innerHTML=inner;
  elem.setAttribute("active",true);
  elem.id=winCount;
  setIcon(programData[id].icon,elem.getElementsByClassName("winicon")[0]);
  ...
  if (id==errID){
    elem.getElementsByClassName("content")[0].innerHTML="<div class=\"errimg\"></div><div class=\"errmsg\">"+contStr+"</div>";
    ...
    elem.getElementsByClassName("content")[0].innerHTML=document.getElementById("settingsbuffer").innerHTML;
    elem.setAttribute("type","settings");
    elem.classList.add("icons_hidden");
  }else if (id==explorerID){
    elem.getElementsByClassName("content")[0].innerHTML=document.getElementById("explorerbuffer").innerHTML;
    elem.setAttribute("type","explorer");
    elem.getElementsByClassName("path")[0].addEventListener("keydown",function(event){if (event.keyCode==13){parseURL(event.target);}});
    ...
    content.innerHTML=document.getElementById("consolebuffer").innerHTML;
...
function addTaskbarIcon(id,count,actLvl,name,icon,stickied){
  ...
  iconEl.setAttribute("data-title",name);
  iconEl.setAttribute("onclick",actLvl==2?"minWin("+count+")":"selectIcon(event,"+id+",true)");
  iconEl.innerHTML="<div class=\"smallicon\"></div>";
  iconEl.addEventListener("mousedown",function(event){showContext(event,id<errID?(id==viewerID||id==consoleID?5:1):4)});
  setIcon(icon,iconEl.getElementsByClassName("smallicon")[0]);
  if (!stickied){
    document.getElementById("taskbar").appendChild(iconEl);
  }else{
    document.getElementById("taskbar").insertBefore(iconEl,elems[taskArr.length+permaStickied]);
  }
}
...
function closeWinHelper(id,actWin){
  ...
  //actWin.style.transition="all 300ms";
  /*var wins=document.getElementsByClassName("window");
  var actWin=null;
  for (var i=0;i<wins.length;i++){
    if (wins[i].id==id){
...
function setName(evt,id){
  ...
  if (evt.keyCode==13){
    ...
    for (var i=0;i<icons.length;i++){
      if (icons[i].getAttribute("data-title")==oldName){
        icons[i].setAttribute("data-title",name);
      }
    }
...
function showHome(evt){
  ...
  setTimeout(function(){
    ...
    var piecesNo=Math.random()*10+10,tot=0,max=5;
    var pb=document.getElementById("puzzlebar");
    for (var i=0;i<piecesNo;i++){
...
function restart(){
  ...
  elem.innerHTML="";
  var inner="<p id=\"loadmsg\">Shutting down...</p><div id=\"loadercontainer\"><div class=\"loader\"></div><div class=\"loader2\"></div><div class=\"loader3\"></div>";
...
function contextPin(type){
  ...
  if (type==0){addTaskbarIcon(id,"null",0,programData[id].name,programData[id].icon,true);}
    ...
    document.getElementById("taskbar").insertBefore(icon,document.getElementsByClassName("taskbaricon")[taskArr.length+permaStickied]);
...
function openSettings(scrTo){
  hideSearch();
  addWindow(setID,null,null,60,35);
  var dt=new Date();
  ...
  colorSelect();
  if (scrTo!=null){
    window.getElementsByClassName("content")[0].scrollTop=document.getElementById(scrTo).offsetTop;
  }
}
